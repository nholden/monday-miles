...
RSpec.describe "Strava webhook callback" do
  ...
  it "creates new activities" do
    ...
    expect(StravaActivityWorker.jobs.size).to eql 1
    worker_args = StravaActivityWorker.jobs.last['args']
    expect(worker_args[0]).to eql 123456
    expect(worker_args[1]).to eql 12345678
    expect(response.status).to eql 200
  end

  it "updates existing activites" do
    params = JSON.parse(file_fixture('strava/updated_activity_webhook_event.json').read)
    params['owner_id'] = 123456
    params['object_id'] = 12345678
    post strava_webhook_callbacks_path, params: params
  ...
  it "deletes existing activities" do
    params = JSON.parse(file_fixture('strava/deleted_activity_webhook_event.json').read)
    params['owner_id'] = 123456
    params['object_id'] = 12345678
    post strava_webhook_callbacks_path, params: params

    expect(DeletedStravaActivityWorker.jobs.size).to eql 1
    worker_args = DeletedStravaActivityWorker.jobs.last['args']
    expect(worker_args[0]).to eql 123456
    expect(worker_args[1]).to eql 12345678
    expect(response.status).to eql 200
  end
